...
public class InjectorContainer extends Container {
	...
	public void detectAndSendChanges() 
	{
		...
		for(int i = 0; i < this.listeners.size(); ++i) 
		{
			...
			if(this.nutrient != this.tileEntity.getField(3)) listener.sendWindowProperty(this, 3, this.tileEntity.getField(3));
		}
        
		this.energy = this.tileEntity.getField(0);
		this.water = this.tileEntity.getField(1);
		this.progress = this.tileEntity.getField(2);
		this.nutrient = this.tileEntity.getField(3);
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (fromSlot != null && fromSlot.getHasStack()) {
			...
			// If it's from the injector, put in player's inventory
			if (index < 2) {
